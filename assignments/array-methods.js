...
runners.map(function(runners){
  return firstNamesAllCaps.push(`${runners.first_name.toLocaleUpperCase()}`)
})
...
runners.filter(function(runners){
  ...
  return runnersLargeSizeShirt.push(`${runners.first_name.toLocaleUpperCase()}`)
}})
...
// Problem 1
// the Company organize needs a list of companie names to add to their banner
let companyNames = [];
runners.forEach(function(runners){
  return companyNames.push(`${runners.company_name}`);
})
